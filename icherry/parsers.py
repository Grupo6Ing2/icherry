...
class CadenaAFechaYHora(Parser):
    ...
    def parse(self, unaCadena):
        fecha = datetime.datetime.strptime(unaCadena.split()[0], "%Y-%m-%d").date()
        hora = datetime.datetime.strptime(unaCadena.split()[1], "%H:%M:%S.%f").time()
...
class ParserPronosticoMeteorologico(Parser):
    ...
    def parse(self, unaCadena):
        ...
        for i in range(0, len(datosSerializados)-1, 6):
            ...
            temp = TemperaturaEnCelsius(CadenaANumero().parse(datosSerializados[i+2]))
            lluvia = CadenaAPorcentaje().parse(datosSerializados[i+3])
            humedad = HumedadRelativa(CadenaAPorcentaje().parse(datosSerializados[i+4]))
            luz = LuzEnLux(CadenaANumero().parse(datosSerializados[i+5]))

            predicciones.append(PrediccionMeteorologica(Rango(desde, hasta), temp, lluvia, humedad, luz))
